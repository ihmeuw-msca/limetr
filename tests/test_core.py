...
def data():
    ...
    return Data(obs=obs, obs_se=obs_se, group_sizes=group_sizes)
...
@pytest.mark.parametrize(
    "my_fevar", [FeVariable(LinearMapping(np.ones((4, 3))))]
)
...
@pytest.mark.parametrize(
    "my_revar", [ReVariable(LinearMapping(np.ones((4, 3))))]
)
...
def test_objective(model, var):
    ...
    tr_obj = 0.5 * (
        np.sum(model.get_residual(beta) ** 2 / model.get_obsvar())
